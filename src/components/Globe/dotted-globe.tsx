  ...
  useEffect(() => {
    ...
    const pointInFeature = (point: [number, number], feature: any): boolean => {
      ...
      if (geometry.type === "Polygon") {
        ...
        if (!pointInPolygon(point, coordinates[0] as [number, number][])) {
          return false
        }
        // Check if point is in any hole (inner rings)
        for (let i = 1; i < coordinates.length; i++) {
            if (pointInPolygon(point, coordinates[i] as [number, number][])) {
        ...
        for (const polygon of geometry.coordinates) {
          ...
          if (pointInPolygon(point, polygon[0] as [number, number][])) {
            ...
            for (let i = 1; i < polygon.length; i++) {
               if (pointInPolygon(point, polygon[i] as [number, number][])) {
                inHole = true
                break
              }
            }
